...
- `netcat-openbsd` をインストールする。
- 自己署名証明書を生成する。
- `nginx.conf` を配置する。
- `entrypoint.sh` で `wordpress:9000` を待つ。
...
| コンテナ | PID 1 になるプロセス |
| --- | --- |
| `nginx` | `entrypoint.sh` が `wordpress:9000` を待ち、最後に `nginx -g daemon off;` |
| `wordpress` | `php-fpm -F` |
...
**Q. なぜ NGINX 側でも `wordpress:9000` を待つのですか。**
A. `depends_on` は WordPress コンテナの起動順を保証しますが、PHP-FPM が `9000` 番で listen したことまでは保証しません。NGINX が先に起動すると、一時的に 502 を返す可能性があります。そこで NGINX の entrypoint で TCP 接続可能になるまで待ってから起動します。
