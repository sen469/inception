...
```sh
mkdir -p secrets
read -rsp 'MariaDB user password: ' DB_PASSWORD && printf '\n'
printf '%s\n' "$DB_PASSWORD" > secrets/db_password.txt
read -rsp 'MariaDB root password: ' DB_ROOT_PASSWORD && printf '\n'
...
- Source walkthrough in Japanese: `docs/inception_manual/source_walkthrough.ja.md`
